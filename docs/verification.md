...
# 更正：V1 的「FTS5 中文子串检索可用」是**错的**（增量，不覆盖以上结论）

**更正日期**：2026-09-12
**触发**：M3 施工时用施工令 §4.2 要求的数据库冒烟测试，原样复刻 V1 的用例，
结果与 V1 记录**不一致**。遂做了 6 个可复现的探测脚本重新查证
（详见 `docs/verify-m3-fts5.md`，用户也要求"再去查证一下"，第二轮已完成）。

## 上面 V1 第 21–22 行应当读作

| V1 原文                               | 实测                                                                                 |
| ------------------------------------- | ------------------------------------------------------------------------------------ |
| `MATCH '加班'` 命中"用户昨天也在加班" | ❌ **0 条**。要 `MATCH '用户昨天也在加班'`（整句）才命中 1 条                        |
| FTS5 默认 `unicode61` 对中文按字切分  | ❌ **按"串"切，不按字切**。unicode61 把连续汉字当**一个 token**，空格/标点才是分隔符 |

V1 的结论并不是笔误——它是**用整句查询**测出来的，于是"能命中"为真，
但从中推出的"子串检索可用"不成立。**这是一个方法学错误：验证时用了与被验证命题不同的输入。**

## 复现（任一脚本，都可独立运行）

```
node scripts/recheck-v1-fts5.mjs      # 原样复刻 V1 用例 → 0 条
node scripts/probe-fts5-chinese.mjs   # 默认 unicode61 的真实分词
node scripts/probe-fts5-trigram.mjs   # trigram 分词器同样漏 2 字词
node scripts/probe-fts5-strategy.mjs  # 5 种策略横向对比：LIKE 5/5，两种 FTS5 均 0/5
node scripts/probe-fts5-deep.mjs      # 前缀查询 / 大小写 / 自定义分词器可行性
node scripts/probe-like-index.mjs     # LIKE 能否走索引 + 真实耗时
```

## 因此对施工令的影响

施工令 §5 M3 要求「FTS5 全文检索」。**它要的"能力"（中文子串检索）是必须的；
"FTS5"这个手段在本机做不到它。** 结论与替代方案见 `docs/DECISIONS.md`，
待你拍板的选项见 `docs/BLOCKERS.md` B-2。

> **对 M0 结论的有效性无影响**：本文其余部分（V2–V5、以及"原生依赖无需 rebuild"
> 这条主结论）均未被推翻——V1 里出错的只有 FTS5 中文检索那一项，
> better-sqlite3 的加载/建表/读写**复测仍然全部通过**。
